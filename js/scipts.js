/** Создаём наше поле и передаём его в документ**/
var crash;
var score_audio = new Audio();
var fon_audio = new Audio();
var lost_audio = new Audio();
score_audio.src = "audio/score.mp3";
fon_audio.src = "audio/fon.mp3";
lost_audio.src = "audio/lost.mp3";
...
/** Создаём 100 ячеек в нашем поле **/
for (let i = 1; i<101; i++){
...
function move (){
	...
	if (direction == 'right'){
		if (snakeCoordinates[0] < 10){
			snakeBody.unshift(document.querySelector('[posX = "' + (+snakeCoordinates[0]+1) + '"][posY = "' +
		 	snakeCoordinates[1] + '"]'));
		} else { snakeBody.unshift(document.querySelector('[posX = "1"][posY = "' +
 			snakeCoordinates[1] + '"]'));
		}
		...
		if (snakeCoordinates[0] > 1){
			snakeBody.unshift(document.querySelector('[posX = "' + (+snakeCoordinates[0] - 1) + '"][posY = "' +
		 	snakeCoordinates[1] + '"]'));
		} else { snakeBody.unshift(document.querySelector('[posX = "10"][posY = "' +
 			snakeCoordinates[1] + '"]'));
		}
		...
		if (snakeCoordinates[1] > 1){
			snakeBody.unshift(document.querySelector('[posX = "' +snakeCoordinates[0] + '"][posY = "' +
		 	(+snakeCoordinates[1]-1) + '"]'));
		} else { snakeBody.unshift(document.querySelector('[posX = "' +snakeCoordinates[0] + '"][posY = "10"]'));
		}
	...
	if (snakeBody[0].classList.contains('snakeBody')) {
		...
		fon_audio.pause();
		setTimeout(() => {
			alert(`Игра окончена, Котозмей съел сам себя! \n Ваши очки: ${score}`);
		...
		snakeBody[0].style.background = 'url(img/scream.jpg) center no-repeat';
